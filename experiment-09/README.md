...
**Note on current status:** the practical steps below have now been actually performed and verified against a running cluster. The Deployment and Service from Experiment 08 were already present in the cluster (5 days old at the time of this experiment), so Experiment 09's own work consisted of checking their state, deliberately deleting the running Pod, and observing Kubernetes recreate it automatically.
